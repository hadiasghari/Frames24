...
# HA from dsets import KnownsDataset
from .tok_dataset import (  # was: from rome.tok_dataset
    TokenizedDataset,
    dict_to_,
...
from . import nethook
# HA from util.globals import DATA_DIR
    ...
    def patch_rep(x, layer):
        if layer == embed_layername:
            ...
            if tokens_to_mix is not None:
                ...
                ).to(dtype=torch.float32, device=x.device)  # HA
                if replace:
                    x[1:, b:e] = noise_data
                else:
                    x[1:, b:e] += noise_data
        ...
        if model is None:
            ...
            if load_in_8bit or load_in_4bit:
                ...
                if torch.backends.mps.is_available():
                    model.eval().to(torch.device("mps"))
                else:
                    model.eval().cuda()
...
def make_inputs(tokenizer, prompts, device=None):
    if not device:
        device = torch.device("mps") if torch.backends.mps.is_available() else torch.device("cuda")  # HA: set to CUDA or Apple MPS
